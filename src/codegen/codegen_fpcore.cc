 ...
 * \file codegen_fpcore.cc
 */
#include <tvm/packed_func_ext.h>
#include <vector>
#include <string>
#include "codegen_fpcore.h"
#include "build_common.h"

namespace tvm {
namespace codegen {

CodeGenFPCore::CodeGenFPCore() {
  module_name_ = GetUniqueName("__tvm_module_ctx");
}

void CodeGenFPCore::Init(bool output_ssa) {
...
void CodeGenFPCore::AddFunction(LoweredFunc f) {
...
std::string CodeGenFPCore::Finish() {
  return CodeGenC::Finish();
}

void CodeGenFPCore::PrintType(Type t, std::ostream& os) {  // NOLINT(*)
  int lanes = t.lanes();
  if (t.is_handle()) {
    CHECK_EQ(lanes, 1)
...
void CodeGenFPCore::VisitExpr_(const Broadcast* op, std::ostream& os) {   // NOLINT(*)
...
void CodeGenFPCore::PrintGetFuncFromBackend(const std::string& func_name,
...
void CodeGenFPCore::PrintFuncCall(const std::string& packed_func_name, int num_args) {
  this->PrintIndent();
...
void CodeGenFPCore::VisitExpr_(const Call *op, std::ostream& os) { // NOLINT(*)
...
void CodeGenFPCore::VisitStmt_(const AssertStmt *op) { // NOLINT(*)
...
void CodeGenFPCore::VisitExpr_(const Min *op, std::ostream& os) {  // NOLINT(*)
  PrintTernaryCondExpr(op, "<", os);
}

void CodeGenFPCore::VisitExpr_(const Max *op, std::ostream& os) {  // NOLINT(*)
  PrintTernaryCondExpr(op, ">", os);
}

template <typename T>
inline void CodeGenFPCore::PrintTernaryCondExpr(const T* op,
                                           const char* compare,
...
runtime::Module BuildFPCore(Array<LoweredFunc> funcs) {
...
TVM_REGISTER_API("codegen.build_fpcore")
.set_body([](TVMArgs args, TVMRetValue* rv) {
    *rv = BuildFPCore(args[0]);
  });
}  // namespace codegen
}  // namespace tvm
